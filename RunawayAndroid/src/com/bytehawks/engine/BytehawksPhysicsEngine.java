 ...
 * 
 */
public class BytehawksPhysicsEngine extends BytehawksObject
{
	public BytehawksVector mGravity;
	public float mViscosity;

	public BytehawksPhysicsEngine(int maxObjects)
	{
		super(maxObjects);
		mGravity = new BytehawksVector();
		mViscosity = 0;
	}
	...
	protected void physics(float secondsElapsed)
	{
		...
		{
			if (mChilds[o] instanceof BytehawksPhysicObject)
			{
				BytehawksPhysicObject mChildO = (BytehawksPhysicObject) mChilds[o];
				// Gravity
			...
			if (mChilds[o] instanceof BytehawksPhysicObject)
			{
				...
				if ((mChildO.mDelta.mX != 0) || (mChildO.mDelta.mY != 0))
				{
					...
					for (int c = 0; c < mChildsCount; c++)
					{
						if (c != o)
						{
							if (mChilds[c] instanceof BytehawksPhysicObject)
							{
								BytehawksPhysicObject mChildC = (BytehawksPhysicObject) mChilds[c];
