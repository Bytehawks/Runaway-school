 ...
 * 
 */
public class BytehawksRotatingSprite extends BytehawksAbstractSprite
{
	...
	/**
	 * 
	 * @param layout BytehawksSpriteLayout
	 */
	public BytehawksRotatingSprite(BytehawksSpriteLayout layout)
	{
		super(layout);
		doInit(0, 0, 1);
	}
	 ...
	 * @param layout BytehawksSpriteLayout
	 */
	public BytehawksRotatingSprite(int x, int y, BytehawksSpriteLayout layout)
	{
		super(layout);
		doInit(x, y, 1);
	}
	 ...
	 * @param layout BytehawksSpriteLayout
	 */
	public BytehawksRotatingSprite(int x, int y, float alpha, BytehawksSpriteLayout layout)
	{
		super(layout);
		doInit(x, y, alpha);
	}
		...
		if (roLayout != null)
		{
			if (roLayout.roTexture != null)
			{
				if (roLayout.roTexture.mHWTextureID > -1)
				{
					...
					if (BytehawksSurfaceView.sUseHWBuffers)
					{
						...
						((GL11) gl).glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER, BytehawksSurfaceView.roIndexBufferIndex);
					...
					else
					{
						...
						mIndexBuffer = ByteBuffer.allocateDirect(BytehawksSurfaceView.sIndexValues.length * 2).order(ByteOrder.nativeOrder())
								.asCharBuffer();
						...
						for (int i = 0; i < BytehawksSurfaceView.sIndexValues.length; ++i)
							mIndexBuffer.put(i, BytehawksSurfaceView.sIndexValues[i]);
						...
						gl.glDrawElements(GL10.GL_TRIANGLES, BytehawksSurfaceView.sIndexValues.length, GL10.GL_UNSIGNED_SHORT, mIndexBuffer);
