 ...
 * 
 */
public class BytehawksString extends BytehawksObject
{
	...
	protected BytehawksFont mFont; // Font
	protected int[] mTextureIV = new int[4]; // Texture coordinates
	public BytehawksVector mPosition; // Position
	...
	/**
	 * 
	 * @param font BytehawksFont
	 */
	public BytehawksString(BytehawksFont font)
	{
		init(font, 3, false);
	}
	 ...
	 */
	public BytehawksString(BytehawksFont font, int tabLength, boolean ignoreNL)
	{
	 ...
	 */
	public BytehawksString(BytehawksFont font, String string, int x, int y, int alignment)
	{
		init(font, 3, false);
		set(string);
		mPosition.set(x,y); 
	...
	private void init(BytehawksFont font, int tabLength, boolean ignoreNL)
	{
		mPosition = new BytehawksVector();
		mFont = font;
		mLength = 0;
		mLinesCount = 0;
		mAlignment = aLeft;
	...
	private int drawLine(GL10 gl, float y, int line)
	{
		if ((line>=0)&&(line<mLinesCount))
		{
			...
			for (int c=mLineStart[line];(c<mLineEnd[line])&&(c<mLength);c++)
			{
				...
				((GL11Ext) gl).glDrawTexfOES(x + mFont.mCharLeft[chr], BytehawksSurfaceView.roHeight - (y + mFont.mHeight + mFont.mLineat),
