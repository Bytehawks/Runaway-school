 ...
 *
 */
public class BytehawksTileMap extends BytehawksObject
{
	protected int[] mTextureIV = new int[4];
	protected BytehawksTileBank mTileBank;
	public BytehawksVector mPosition; 
	...
	public BytehawksTileMap (BytehawksTileBank tileBank, int width, int height, int columnsCount, int rowsCount, boolean horizontalRepeat, boolean verticalRepeat)
	{
		Init (tileBank);
		mWidth=width;
		mHeight=height;
	...
	private void Init(BytehawksTileBank tileBank)
	{
		mTileBank=tileBank;
				   ...
				   for (int y=0;y<ttdY;y++)
				   {
					   ...
					   for (int x=0;x<ttdX;x++)
					   {
					   	...
					   	if ((tile>=0)&&(tile<mTileBank.mTilesCount))
					   	{
					   		...
					   		((GL11Ext) gl).glDrawTexfOES(mPosition.mX+px, BytehawksSurfaceView.roHeight-(mPosition.mY+py+TH), mZ, TW, TH);
					   	}
					   }
				   }
				}
	...
	protected int getTile(int c, int r)
	{
		if (c<0)
		{
