 ...
 * 
 */
public class BytehawksSpriteLayout
{
	public int roWidth; //
	public int roHeight;// Dimensions (ReadOnly)

	public BytehawksTexture roTexture; //Texture (ReadOnly)
	...
	private BytehawksVector[] mPivot; //Pivot point
	
	/**
	 * 
	 * @param view			Main BytehawksSurfaceView
	 * @param width
	 *           Width in pixels
	 * @param height
	 *           Height in pixels
	 ...
	 */
	public BytehawksSpriteLayout(BytehawksSurfaceView view, int width, int height, int resourceId, int cropLeft, int cropTop, int cropWidth,
			int cropHeight)
	{
	 ...
	 *           Resource bitmap
	 */
	public BytehawksSpriteLayout(BytehawksSurfaceView view, int resourceId)
	{
	 ...
	 *           Resource bitmap
	 */
	public BytehawksSpriteLayout(BytehawksSurfaceView view, int width, int height, int resourceId)
	{
		doInit(view, width, height, resourceId, 0, 0, width, height, 1, 1);
	}

	private void doInit(BytehawksSurfaceView view, int width, int height, int resourceId, int cropLeft, int cropTop, int cropWidth, int cropHeight, int frameCount,
			int frameColumns)
	{
		...
		if ((width==0)||(height==0))
		{
			...
			finally
			{
				...
				{
					Log.e("BytehawksTextureEngine", "loadTexture::InputStream.close error: " + e.getMessage());
				}
			}
		}
		else
		...
		mPivot=new BytehawksVector[roFrameCount];
		for (int f=0;f<roFrameCount;f++)
			mPivot[f]=new BytehawksVector(roWidth / 2, roHeight / 2);
	 ...
	 * @return pivot point 
	 */
	public BytehawksVector getPivot(int frame)
	{
		if (frame<roFrameCount)
			return mPivot[frame];
		return null;
